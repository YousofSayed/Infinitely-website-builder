...
export const ScrollableToolbar = ({ 
  ...
  return (
    ...
    >
      {isScrollableElement && (
        <div
          className={`z-50 absolute left-0 top-[-25px] h-[calc(100%+50px)] w-[40px]  bg-blue-900/40 flex items-center justify-center  transition-all `}
        >
          <button
            onClick={(ev) => {
      ...
      {isScrollableElement && (
        <div
          className={`z-50 absolute right-0 top-[-25px] h-[calc(100%+50px)] w-[40px]  bg-blue-900/40 flex items-center justify-center  `}
        >
          <button
            onClick={(ev) => {
              addClickClass(ev.currentTarget, "click");
