...
export const MediaManager = () => {
          ...
          wpWorkerCallbackMaker(
            ...
            (worker_res) => {
              if (worker_res.done) {
                ...
                setMediaFiles(
                  mediaFiles.filter(
                    (item) => !mediaSelected.some((s) => s.id === item.id),
                  ),
                );
                setMediaSelected([]);
                setLoading(false);
                toast.dismiss(tid);
                toast.success(
                  <ToastMsgInfo msg="Files deleted successfully 🎉" />,
                );
                res(worker_res);
              }
            },
  ...
  const getMediaFiles = async () => {
    ...
    runWithBusy(async () => {
      ...
      if (isArray(res)) {
        const willBe = res.filter(
          (item) => !excludes.some((ex) => ex.id === item.id),
        );
        // window.__res = willBe;
  ...
  return (
    <section className=" h-full w-full flex flex-col gap-2 overflow-hidden">
      ...
      <main
        className="w-full h-[calc(100%-50px)] overflow-x-hidden overflow-y-auto rounded-lg"
        ref={animateRef}
      >
        {loading && <Loader />}
            ...
            endReached={async () => {
              runWithBusy(async () => {
                ...
                const tid = toast.loading(
                  <ToastMsgInfo msg={"Loading more files..."} />,
                );
