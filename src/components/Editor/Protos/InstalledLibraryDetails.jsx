...
import { open_file_editor_modal } from "@/constants/InfinitelyCommands";
...
export const InstalledLibraryDetails = ({
  ...
  const executeDeleteLibrary = async () => {
    const cnfrm = confirm(
      `Are you sure to delete ${library.name} library ? 🤔`,
    );
    if (!cnfrm) return;
    // setIsDeleting(true);
    const tid = toast.loading(
      <ToastMsgInfo msg={`Deleting ${library.name} library...`} />,
    );
    try {
      ...
      await doInWordpressAsync(async () => {
        ...
        const cnfrm = confirm(
          `Do you want to delete this library from media library too ? 🤔`,
        );
        if (cnfrm) {
      ...
      setSelected(selected.filter((lib) => lib.name != library.name));
  ...
  const [deleteLibrary, { isLoading: isDeleting }] = useBusyCallback(
    executeDeleteLibrary,
    { key: "delete-library" },
  );

  const { isLoading: isDeletingLibraries } = useTaskState("delete-libraries");
  const { isLoading: isSavingOrders } = useTaskState("save-orders");
