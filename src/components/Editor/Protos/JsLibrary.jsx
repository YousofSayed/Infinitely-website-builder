  ...
  const executeInstall = async ({
    ...
    try {
      ...
      let resBlob = new Blob(
        [
          `/* ${library.name} : ${library.latest} : Added Manually And It Will Be Not Minified */`,
        ],
        { type: isJs ? "application/javascript" : isCss ? "text/css" : "" },
      );
      ...
      if (!library.manually) {
        ...
        const rgx = /export(\s+)/gi;
        if (rgx.test(libContent)) {
          // const cnfrm = alert(
          //   `This is module library , do you accept to transform it to normal library ?`,
          // );
          throw new Error(`This js file not allowed 😑`);
          // if (cnfrm) {
      ...
      !installData.globalName &&
        (installData.globalName =
          installData.globalName || isJs
            ? (await detectGlobalsSandbox(fileUrl))?.[0]
            : "");
      /**
      ...
      console.error(`Failed To Install Library ${library.name}`);
      window._installing_lib = false;
      toast.dismiss(tId);
      toast.error(
        <ToastMsgInfo msg={error.message || `Failed To Install Library`} />,
      );
            ...
            <div className="w-full flex items-start flex-col gap-2 ">
              ...
              <div className="w-full flex items-center justify-between flex-wrap gap-2 bg-surface-secondary p-2 rounded-lg">
                {(library?.fileType == "js" ||
                  ...
                  <>
                    <fieldset>
                      <FitTitle className="text-lg font-semibold mb-2">
                        Load in
                      </FitTitle>
